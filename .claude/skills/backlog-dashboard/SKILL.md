...
description: >
  ...
  Do NOT use for scanning new repos (use repo-scan), applying fixes (use apply-backlog-item), or reviewing code.
metadata:
  author: phmatray
  version: 2.0.0
---
...
- See `../shared/gh-prerequisites.md` for authentication and error handling details.
...
Tier point values are defined in `../shared/backlog-format.md#tier-system`.
...
Result: Multi-repo overview table showing health scores and issue counts for all scanned repos, followed by a recommendation for the highest-impact next action.

**Example 2: Drill into one repo**
User says: "backlog status for phmatray/NewSLN"
Result: Detailed single-repo view with remaining health items by priority, completed items, score breakdown by tier with progress bars, and open issues table.

**Example 3: Find next action**
User says: "what should I work on next?"
Result: Dashboard with recommendation pointing to the lowest-scoring repo's highest-priority failing item, with the exact command to apply it.

## Troubleshooting

**"No backlog directory found"**
No repos have been scanned yet. Run `/repo-scan owner/repo` first.

**Scores don't match expectations**
Re-run the scan to refresh: `/repo-scan owner/repo`. Scores are based on the last scan's SUMMARY.md.
