...
Shared prerequisites, repo detection, context detection, and common edge cases for all skills. Consumed by: repo-scan, apply-backlog-item, backlog-dashboard.
...
If `gh repo view` shows the repo is archived, warn the user that changes cannot be pushed. Health scanning is still useful for informational purposes, but apply-backlog-item will not work.
